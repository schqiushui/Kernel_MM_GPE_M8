 ...
 * @min_voltage_mv:		min battery voltage sch before turning the FET on
	...
	if (is_vin_min_detected) {
		...
		if (aicl_timer.total_time_ms >= VIN_MIN_DETECT_DURATION_MS) {
			...
			if (usb_ma >= usb_target_ma) {
				...
				if (!is_usb_target_ma_changed_by_qc20 && gs_stop_eoc_cable_impedance == false) { /* Not QC2.0 charger */
...
qpnp_eoc_work(struct work_struct *work)
{
	...
	if ((batt_sts & BAT_FET_ON_IRQ) && (chg_sts & FAST_CHG_ON_IRQ
		...
		pr_debug("ibat_ma = %d vbat_mv = %d term_current_ma = %d, is_batt_full_eoc_stop = %d\n",
