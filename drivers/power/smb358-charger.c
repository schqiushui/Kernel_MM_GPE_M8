...
#define CHG_MIN_FLAG 0
#define HIGHVOL_MIN_FLAG 1
				...
				struct smb358_charger *chip)
{
	...
	if (chg_limit_current[CHG_MIN_FLAG] != 0 && chg_limit_current[HIGHVOL_MIN_FLAG] != 0){
		now_chg_limit_current = min(chg_limit_current[CHG_MIN_FLAG], chg_limit_current[HIGHVOL_MIN_FLAG]);
		pr_info("charge limit value now is %d\n", now_chg_limit_current);
		current_max = min(current_max, now_chg_limit_current);

	}else if(chg_limit_current[CHG_MIN_FLAG] != 0){
		current_max = min(current_max, chg_limit_current[CHG_MIN_FLAG]);

	}else if(chg_limit_current[HIGHVOL_MIN_FLAG] != 0){
		current_max = min(current_max, chg_limit_current[HIGHVOL_MIN_FLAG]);
	}
...
int smb358_set_limit_charge_on_high_vol_enable(bool enable)
{
	...
	if(enable)
		chg_limit_current[HIGHVOL_MIN_FLAG] = SMB358_CHG_I_HIGHVOL_MIN_MA;
	else
		chg_limit_current[HIGHVOL_MIN_FLAG] = 0;
...
int smb358_limit_charge_enable(bool enable)
{
	...
	if (enable)
		chg_limit_current[CHG_MIN_FLAG] = SMB358_CHG_I_MIN_MA;
	else
		chg_limit_current[CHG_MIN_FLAG] = 0;
